...
import { localizeScale } from '../lib/agentUtils';
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <p className="text-gold text-sm font-body font-medium tracking-widest uppercase">
              {t('agent.eyebrow')}
            </p>
          </div>
          ...
          <p className="text-foreground-muted font-body text-lg">{t('agent.subtitle')}</p>
        </div>

        {/* How it works strip */}
        <div
          className="glass-card p-5 mb-8 grid grid-cols-2 md:grid-cols-4 gap-4"
          aria-busy={agentInfoLoading}
        >
          {agentInfoLoading
            ...
            : [
                ...
                {
                  icon: Activity,
                  label: t('agent.strip.datasetsQueried'),
                  value: t('agent.strip.sellersValue'),
                },
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card-gold p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-5">
                <div>
                  <p className="text-xs text-foreground-muted font-body mb-1">
                    {t('common.labels.protocol')}
                  </p>
                  <p className="font-display font-semibold text-foreground">
                    {result.report.topOpportunity.protocol}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-foreground-muted font-body mb-1">
                    {t('agent.metrics.vaultPool')}
                  </p>
                  <p className="font-body font-medium text-foreground text-sm">
                    {result.report.topOpportunity.vault}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-foreground-muted font-body mb-1">
                    {t('common.labels.chain')}
                  </p>
                  <p className="font-body font-medium text-foreground text-sm">
                    {result.report.topOpportunity.chain}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-foreground-muted font-body mb-1">
                  ...
                  <p className="font-display font-bold text-gold text-xl">
                    {result.report.topOpportunity.apy}%
                  </p>
                </div>
                <div>
                  <p className="text-xs text-foreground-muted font-body mb-1">
                    {t('common.labels.riskLevel')}
                  </p>
                  <p
                    className={clsx(
                      'font-body font-semibold text-sm',
                      RISK_COLOR[result.report.topOpportunity.riskLevel] ?? 'text-foreground',
                    )}
                  >
                    {ls(result.report.topOpportunity.riskLevel)}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-foreground-muted font-body mb-1">
                    {t('common.labels.whaleConfidence')}
                  </p>
                  <p
                    className={clsx(
                      'font-body font-semibold text-sm',
                      CONFIDENCE_COLOR[result.report.topOpportunity.whaleConfidence] ??
                        'text-foreground',
                    )}
                  >
                    {ls(result.report.topOpportunity.whaleConfidence)}
                  </p>
                </div>
              </div>

              <div className="border-t border-border/40 pt-4">
                <p className="text-xs text-foreground-muted font-body mb-2">
                  {t('common.labels.sentiment')}
                </p>
                <span
                  className={clsx(
                    'text-sm font-body font-semibold',
                    SENTIMENT_COLOR[result.report.topOpportunity.sentimentScore] ??
                      'text-foreground',
                  )}
                >
                  {ls(result.report.topOpportunity.sentimentScore)}
                </span>
              </div>
            </div>
                ...
                {result.report.alternatives.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {result.report.alternatives.map((alt, i) => (
                      <li
                        key={i}
                        className="text-sm font-body text-foreground-muted leading-relaxed flex gap-2"
                      >
                ...
                {result.report.warnings.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {result.report.warnings.map((w, i) => (
                      <li
                        key={i}
                        className="text-sm font-body text-amber-300 leading-relaxed flex gap-2"
                      >
            ...
            <div className="glass-card p-6">
              <h3 className="font-display font-semibold text-foreground mb-3">
                {t('agent.result.fullAnalysis')}
              </h3>
              <p className="font-body text-foreground-muted text-sm leading-relaxed">
                {result.report.rawAnalysis}
              </p>
            </div>
            ...
            <div className="glass-card p-6">
              ...
              <div className="space-y-3 mb-4">
                {result.payments.sellerPayments.map(p => {
                  ...
                  const badgeClass =
                    typeColors[p.type] || 'bg-surface-2 text-foreground-muted border-border';
                  return (
                    <div
                      key={p.txHash}
                      className="flex items-center gap-3 p-3 rounded-xl bg-surface-2/40 border border-border/30"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {!p.onChain && (
                            <span className="px-1.5 py-0.5 rounded text-[10px] font-body bg-amber-400/10 text-amber-400">
                              {t('common.labels.simulated')}
                            </span>
                          )}
                        </div>
                      ...
                      <p className="text-sm font-display font-bold text-gold flex-shrink-0">
                        {p.amount} USDC
                      </p>
                    </div>
                  );
